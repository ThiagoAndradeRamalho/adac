...
const Tarefas = () => {
  ...
  useEffect(() => {
    async function fetchTarefas() {
      try {
        const response = await fetch("https://adacc.onrender.com1/api/tarefa");
        if (response.ok) {
  ...
  async function handleStatusUpdate(tarefaId, newStatus) {
    try {
      const response = await fetch(`https://adacc.onrender.com1/api/tarefa/${tarefaId}/status`, {
        method: "PUT",
  ...
  async function handleOrderUpdate(tarefasAtualizadas) {
    ...
    try {
      const response = await fetch("https://adacc.onrender.com1/api/tarefa/update-ordem", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
