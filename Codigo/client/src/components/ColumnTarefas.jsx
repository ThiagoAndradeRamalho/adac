...
const Column = ({ title, tarefas, setTarefas, status }) => {
  ...
  const handleFormSubmit = async () => {
    ...
    try {
      const response = await fetch("https://adacc.onrender.com0/api/tarefa", {
        method: "POST",
        headers: {
  ...
  const handleFormUpdate = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://adacc.onrender.com0/api/tarefa/${tarefaToUpdate.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleFormDelete = async () => {
    try {
      const response = await fetch(
        `https://adacc.onrender.com0/api/tarefa/${tarefaToDelete.id}`,
        {
          method: "DELETE",
        }
      );
  ...
  useEffect(() => {
    async function fetchUsers() {
      try {
        const response = await fetch("https://adacc.onrender.com0/api/usuario");
        if (response.ok) {
          const data = await response.json();
          setUsuarios(data);
        } else {
